...
public class QuorumConfigBuilder implements Closeable
{
    ...
    public QuorumPeerConfig buildConfig(int instanceIndex) throws Exception
    {
        ...
        if ( isCluster )
        {
            ...
            {
                properties.setProperty("server." + thisSpec.getServerId(), String.format("%s:%d:%d", thisSpec.getHostname(), thisSpec.getQuorumPort(), thisSpec.getElectionPort(), thisSpec.getPort()));
            }
        }
